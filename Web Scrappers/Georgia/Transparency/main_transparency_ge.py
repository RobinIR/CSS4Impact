...
def transparency_ge(webpage, start_page_number,end_page_number, category_name, keyword, search_keyword,data_found=False):   
   try:
      next_page = webpage +"&keyword="+search_keyword+"&page="+ str(start_page_number)
      ...
      try:
         ...
         next_button= soup.find('',class_='pager-next')
         # Extracting urls
         articles = soup.findAll('li', class_="search-result") 
         if len(articles) == 0:
            if not data_found:
               print("No data found")
            return
         ...
         for link in articles:
            try:
               url = f"https://transparency.ge{link.find('a')['href']}"
               date = ""
               language ="Georgian"
         ...
         if next_button:   
            if start_page_number < end_page_number:
               ...
               transparency_ge(webpage, start_page_number,end_page_number, category_name, keyword, search_keyword,True)
               return
            else:
               return
      except:
...
for item in all_category_data:
    ...
    for idp in idps:
      for keyword in keywords:
            key = f"{idp}{keyword}"
            webpage = f"https://transparency.ge/ge/search?type=All&topic=All" 
            search_keyword = urllib.parse.quote(key)
            transparency_ge(webpage, start_page_number,end_page_number, category_name, keyword, search_keyword)
