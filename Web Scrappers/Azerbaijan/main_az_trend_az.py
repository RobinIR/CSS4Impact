...
def infinit_scrolling_for_az_trend_z(url, start_page_number, end_page_number, category, keyword, search_keyword):
        ...
        c = 0
        for link in soup.find('ul', class_="news-list"):
            a = link.find('a')    
            try:          
                ...
                if 'href' in a.attrs:              
                    ...
                    url = a.get('href')
                    date = link.find('span', class_='date-time').text.strip()
    ...
    # Generating the next page url
        ...
        if start_page_number < end_page_number:
            ...
            infinit_scrolling_for_az_trend_z(webpage, start_page_number, end_page_number, category, keyword, search_keyword)
    ...
    if categoryId:
        for idp in idps:
            for keyword in keywords:
                ...
                webpage = f"https://az.trend.az/search?query={search_keyword}"
                infinit_scrolling_for_az_trend_z(webpage, start_page_number,end_page_number, category_name, keyword, search_keyword)